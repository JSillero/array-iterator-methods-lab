...
firstLast = inventors.map(({ first, last }) => ([first + " " + last]));
...
isEveryone19OrOlder = devs.every(({ year }) => new Date().getFullYear() - year > 18);
...
commentById = comments.find(comm => comm.id === 823423);
...
totalYearsLived = inventors.reduce((accumulator, element) => accumulator + (element.passed - element.year), 0);
...
travelMethodCounts = travelMethods.reduce((accumulator, element) => {
